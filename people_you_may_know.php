...
<html lang="en">
   ...
   <body>
      <ul class="fixedElement">
         ...
         <li>
            <a href="people_you_may_know.php?person_id=<?php echo $person_id ?>"><font color=#c4a747>People you may know</font></a>
         </li>
         <li>
         ...
         <table>
            <thead class >
               <tr class="table100-head" >
                  ...
                  <th class="column3">NUMBER OF POSTS</th>
                  <th class="column4">MUTUAL FRIENDS</th>
               </tr>
            </thead>
            <tbody>
               <?php
                  ...
                  exit();
                    ...
                    $s1="select distinct person2_id from friendship where person2_id != '$id' and person1_id in(
                      select person2_id from friendship where person1_id='$id'
                      ) and person2_id not in (select person2_id from friendship where person1_id='$id')";
                    $ss1=mysqli_query($mysqli,$s1);
                    $count=0;
                  while($sss1=mysqli_fetch_row($ss1))
                  {
                  
                    $count++;
                    $know_id=$sss1[0];
                    $s2="select distinct person2_id from friendship where person1_id='$id' and person2_id in(select person2_id from friendship where person1_id='$know_id')";
                    $ss2=mysqli_query($mysqli,$s2);
                    $mutual_no=0;
                    while($sss2=mysqli_fetch_row($ss2))
                    {
                      $mutual_no++;
                      $mutual_id=$sss2[0];
                    }
                    $s3="select name,gender from person where person_id='$know_id'";
                    $ss3=mysqli_query($mysqli,$s3);
                    $sss3=mysqli_fetch_row($ss3);
                    $know_name=strtoupper($sss3[0]);
                    $s4="select name from person where person_id='$mutual_id'";
                    $ss4=mysqli_query($mysqli,$s4);
                    $sss4=mysqli_fetch_row($ss4);
                    $mutual_name=strtoupper($sss4[0]);
                    $stmt3="SELECT count(person_id) from post where person_id='$know_id';";
                    $query3=mysqli_query($mysqli,$stmt3);
                    $post=mysqli_fetch_row($query3);
                  ?>
               <tr>
                  <td class="column1"><a href="profile.php?person_id=<?php echo $id ?>&view_id=<?php echo $know_id ?>"><?php echo $know_name?></a> </td>
                  <td class="column2"><?php $r=strtoupper($sss3[1]); echo $r ?></td>
                  <td class="column3"><?php echo $post[0] ?></td>
                  <td class="column4"><?php echo $mutual_name?>  <?php if($mutual_no>1)
                     {
                         echo " and "; echo $mutual_no-1;?> other<?php if($mutual_no>2){
                     echo "s";
                     }
                     }
                     ?></td>
