...
<body>
	...
	<ul class="fixedElement">
		...
		?>
		 ...
		 <li>
			<a href="friends.php?person_id=<?php echo $person_id ?>"><font color=#c4a747>Friends</font></a>
		 </li>
  ...
  <table>
    <thead class >
      <tr class="table100-head" >
        ...
        <th class="column4">NUMBER OF FRIENDS</th>
      </tr>
    </thead>
    <tbody>
...
$stmt = "SELECT name,person_id,gender FROM person where person_id != '$id' and person_id in (select person2_id from friendship where person1_id='$id')  order by person_id desc";
//first pass just gets the column names
//print "<table> n";
$querry=mysqli_query($mysqli,$stmt);
$count=0;
while ($row = mysqli_fetch_row($querry)) {
	$count++;
  $stmt2="SELECT count(person1_id) from friendship where person1_id='$row[1]';";
  $querry2=mysqli_query($mysqli,$stmt2);
  $row2=mysqli_fetch_row($querry2);
  $stmt3="SELECT count(person_id) from post where person_id='$row[1]';";
...
?>
								...
								<tr>
									<td class="column1"><a href="profile.php?person_id=<?php echo $id ?>&view_id=<?php echo $view ?>"><?php $rr=strtoupper($row[0]); echo $rr?></a> </td>
									<td class="column2"><?php $r=strtoupper($row[2]); echo $r ?></td>
									<td class="column3"><?php echo $post[0] ?></td>
									<td class="column4"><?php echo $row2[0] ?></td>
								</tr>
...
if(!$count)
{
  ?>
  <br><br><br><br>
  <center><h1>IT'S LONELY HERE</h1></center>
  <br><br><br><br>
  <?php
}
?>
